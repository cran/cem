`relax.cem` <-
function(obj, data, depth=1, verbose = 1, L1.breaks=NULL, plot=TRUE, fixed=NULL, 
shifts=NULL, minimal=NULL, use.coarsened=TRUE, ...)
{
    ...
    if(is.null(L1.breaks)){
	  ...
	  if(is.null(L1.breaks)){
       ...
       for(i in 1:nv){
       L1.breaks[[i]] <- pretty(range(data[i],na.rm=TRUE), n=nclass.scott(data[[i]]), 1)
    ...
    for(i in v.num){
     ...
     if(!is.null(obj$breaks[[ vna ]]) || 
	   (is.null(obj$breaks[[ vna ]]) 
#	   && ( is.numeric(data[[ vna ]]) || is.integer(data[[ vna ]]) ) 
		)){
	   if(!is.null(obj$breaks[[ vna ]])){	
	    ...
	    if(use.coarsened)
	     ...
	     n.br <- nclass.FD( as.numeric( data[[ vna ]]) )
	   }
	   if(n.br <= 1)
	   ...
	   if(n.br>1){
		...
		if(use.coarsened)
		 ...
		 len.uni <- length(unique(as.numeric(data[[ vna ]])))
  ...
  tab <- as.data.frame(matrix(0,n.comb+1,2*obj$n.groups+2+1))
	colnames(tab) <- c(paste("G",obj$g.names,sep=""), 
	     paste("PercG",obj$g.names,sep=""),  "Relaxed","L1","var")
	...
	tab[1,"L1"] <- L1.meas(obj$groups[IDX], L1data[IDX,obj$vars], L1.breaks)$L1
  last <- 0
  k <- 1
  K.tab <- 2
  adv <-0
  for(r in 1:n.relax){
   ...
   for(i in 1:r1){
	...
	tab[K.tab,"L1"] <- L1.meas(obj$groups[IDX], L1data[IDX,tmp.obj$vars], L1.breaks)$L1
    tab[K.tab, "var"] <- vna
	tmp.tab <- cem.summary(obj=tmp.obj, verbose = verbose)
	
	
	tab[K.tab,1:(2*obj$n.groups)] <- as.numeric(c(tmp.tab[2,], tmp.tab[2,]/tmp.tab[1,]*100))
	r.str <- NULL

	for(v in 1:c1){
	  r.str <- c(r.str, sprintf("%s(%d)", colnames(brk)[v], brk[i,v]-1))
	}
...
# don't know what to do here with coarsened data
	 s.tmp.obj <- shift.cem(tmp.obj, data=data, shifts=shifts, verbose=verbose, plot=FALSE) 
	 IDX <- which(s.tmp.obj$matched)
 	 tab[K.tab,"L1"] <- L1.meas(obj$groups[IDX], L1data[IDX,s.tmp.obj$vars], L1.breaks)$L1
	...
	out[[1]] <- tab
	for(i in 2:obj$n.groups)
		out[[i]] <- tab[order(tab[,i]),]
   class(out) <- "relax.cem"
